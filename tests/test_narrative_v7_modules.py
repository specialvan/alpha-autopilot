...
def test_benchmark_store_auto_remediates_governance_escalation_remediation_auto_remediate_runs(
    ...
    orchestrator_run_history_prune_dry_run = (
        store.prune_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            keep_last=1,
            dry_run=True,
        )
    )
    ...
    orchestrator_run_history_prune_apply = (
        store.prune_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            keep_last=1,
            dry_run=False,
        )
    ...
    orchestrator_run_history_after_prune = (
        store.list_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            limit=20
        )
    )
    ...
    orchestrator_run_history_auto_prune_dry_run = (
        store.auto_prune_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            dry_run=True
        )
    )
    ...
    orchestrator_run_history_auto_prune_apply = (
        store.auto_prune_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            dry_run=False
        )
    )
    ...
    orchestrator_run_history_after_auto_prune = (
        store.list_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            limit=20
        )
    )
    ...
    threshold_auto_prune = (
        store.auto_prune_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            dry_run=True
        )
    )
    assert threshold_auto_prune.should_prune is True
    assert threshold_auto_prune.prune is not None
    assert threshold_auto_prune.malformed_line_count == 0

    monkeypatch.setenv(
        "AA_V7_BENCH_GOVERNANCE_ESCALATION_REMEDIATION_AUTO_REMEDIATE_RUN_AUTO_REMEDIATE_RUNS_PRUNE_TRIGGER_COUNT",
        "100",
    )
    auto_prune_not_needed = (
        store.auto_prune_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            dry_run=True
        )
    )
    ...
    auto_remediate_prune_apply = (
        store.auto_remediate_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            dry_run=False,
            limit=20,
        )
    )
    assert auto_remediate_prune_apply.action == "auto_prune_runs"
    assert auto_remediate_prune_apply.executed is True
    assert auto_remediate_prune_apply.remediated_orchestrator_runs is False
    assert auto_remediate_prune_apply.pruned_run_history is True
    assert auto_remediate_prune_apply.orchestrator_runs_auto_remediate is None
    assert auto_remediate_prune_apply.run_history_auto_prune is not None
    assert auto_remediate_prune_apply.run_history_auto_prune.prune is not None
    assert auto_remediate_prune_apply.run_history_auto_prune.prune.malformed_dropped_count >= 1
    assert auto_remediate_prune_apply.message == "remediated"

    monkeypatch.setenv(
        "AA_V7_BENCH_GOVERNANCE_ESCALATION_REMEDIATION_AUTO_REMEDIATE_RUN_AUTO_REMEDIATE_RUNS_STALE_SECONDS",
        "0",
    )
    auto_remediate_orchestrator_dry_run = (
        store.auto_remediate_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            dry_run=True,
            limit=20,
        )
    )
    assert auto_remediate_orchestrator_dry_run.action == "run_auto_remediation_orchestrator_runs"
    assert auto_remediate_orchestrator_dry_run.executed is False
    assert auto_remediate_orchestrator_dry_run.remediated_orchestrator_runs is False
    assert auto_remediate_orchestrator_dry_run.pruned_run_history is False
    assert auto_remediate_orchestrator_dry_run.orchestrator_runs_auto_remediate is not None
    assert auto_remediate_orchestrator_dry_run.run_history_auto_prune is None
    assert auto_remediate_orchestrator_dry_run.message == "dry_run"

    monkeypatch.setenv(
        "AA_V7_BENCH_GOVERNANCE_ESCALATION_REMEDIATION_AUTO_REMEDIATE_RUN_AUTO_REMEDIATE_RUNS_STALE_SECONDS",
        "3600",
    )
    auto_remediate_observe_apply = (
        store.auto_remediate_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs(
            dry_run=False,
            limit=20,
        )
    )
    assert auto_remediate_observe_apply.action == "observe"
    assert auto_remediate_observe_apply.executed is False
    assert auto_remediate_observe_apply.remediated_orchestrator_runs is False
    assert auto_remediate_observe_apply.pruned_run_history is False
    assert auto_remediate_observe_apply.orchestrator_runs_auto_remediate is None
    ...
    auto_remediate_runs_auto_remediate_runs_history = (
        store.list_maintenance_alert_governance_escalation_remediation_auto_remediate_run_auto_remediate_runs_auto_remediate_runs_auto_remediate_runs(
            limit=20
        )
    )
    assert auto_remediate_runs_auto_remediate_runs_history.total_records == 3
    assert any(
        item.action == "auto_prune_runs" for item in auto_remediate_runs_auto_remediate_runs_history.records
    )
    assert any(
    ...
    assert any(item.action == "observe" for item in auto_remediate_runs_auto_remediate_runs_history.records)
