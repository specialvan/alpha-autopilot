...
SCRIPT_PATH = Path(__file__).resolve().parents[1] / "scripts" / "run_v6_acceptance_review.py"


def load_module():
    spec = importlib.util.spec_from_file_location("run_v6_acceptance_review", SCRIPT_PATH)
...
def test_v6_acceptance_review_commands_cover_required_gates() -> None:
    module = load_module()
...
def test_v6_acceptance_review_run_writes_report(monkeypatch, tmp_path) -> None:
    module = load_module()
    ...
    assert "V6 Acceptance Review Report" in report_text
    assert "v6_targeted_gate" in report_text
