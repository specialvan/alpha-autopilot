...
def _commands(repo_root: Path) -> tuple[AcceptanceCommand, ...]:
    return (
        ...
        AcceptanceCommand(
            ...
            cwd=repo_root,
            argv=("npm", "--prefix", "ui-react", "run", "build"),
        ),
    )
...
def _default_report_path(repo_root: Path, started_at: datetime) -> Path:
    ...
    return repo_root / "artifacts" / "acceptance" / f"v6-acceptance-{timestamp}.md"
...
def _write_report(
    ...
    *,
    ...
    lines: list[str] = []
    lines.append("# V6 Acceptance Review Report")
    lines.append("")
...
def run(
    *,
    ...
    for command in commands:
        print(f"[v6-acceptance] {command.name}: {command.display()} (cwd={command.cwd})", file=out)
    ...
    print(f"[v6-acceptance] report: {report_path}", file=out)
    return exit_code, report_path


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run V6 acceptance gates and write a markdown report.")
    parser.add_argument(
        "--repo-root",
        type=Path,
        default=REPO_ROOT,
